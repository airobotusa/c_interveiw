...
void printLinkedList(product *pProduct){
	...
	while(pProduct != NULL) {
		...
		printf("A %s of Brand : %s, costs %.2f\n\n",
			(*pProduct).name,
			pProduct->brand,
			pProduct->price);
...
void main(){
	...
	product tomato = {"Tomato","A", .51, NULL};
	product potato = {"Potato", "B",1.21, NULL};
	product lemon = {"Lemon", "C", 1.69, NULL};
	product milk = {"Milk", "A", 3.09, NULL};
	product turkey = {"Turkey","B", 1.86, NULL};
	...
	product apple = {"Apple","A", 1.58, NULL};
